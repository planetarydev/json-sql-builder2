...
class SQLOperator {
	 ...
	 */
	registerPrivateHelper(helperName) {
		let hn = '$' + helperName,
			modulePath,
			privateHelper;

		// build the path for an operator
		modulePath = path.join(this.__builderInstance__.currentModulePath, `./private/${helperName}/${helperName}.js`);
		try {
			privateHelper = require(modulePath).definition;
		} catch (err) {
	...
	//
	Keyword(keyword) {
		this.__builderInstance__.defineKeyword(keyword);
	}
	...
	// __getTypeBasedCompilation__
	//
	// Returns the compilation of a Type-Based Syntax for the given type
	// or null if it's not allowed by definition
	//
	// @return {Object}		compiled/translated syntax and helpers
	//
	__getTypeBasedCompilation__(type) {
		return this.__hasTypeBasedSyntax__(type) ? this.__compiled__[type].translated : null;
	}
	...
	__checkQueryType__(query, allowedTypes, eachItemOfParentType) {
		...
		_.forEach(allowedTypes, (value, key) => {
			...
			if (this.__builderInstance__[validateFunction](query)) {
				validType = key;
			}
		});
		...
		if (!validType) {
			...
			if (eachItemOfParentType){
				...
				throw new Error(`Using Helper '${this.__name__}' must be type of '${Object.keys(allowedTypes).join(', ')}' bot got Type '${invalidType}' with value '${JSON.stringify(query)}'`);
			}
		}
		return validType;
	}

	linkOperators(query, registeredHelpers) {
		_.forEach(registeredHelpers, (helper, helperName) => {
			if (this.__builderInstance__._operators[helperName]) {
				this.__builderInstance__._operators[helperName].link.call(this.__builderInstance__, query);
			}
		});
	}
	...
	__build__(query, identifier) {
		...
		if (syntax) {
			this.linkOperators(query, syntax.__registeredHelpers__);
		...
		if (!syntax) {
			...
			if (type == 'Array' || type == 'Object'){
				...
				_.forEach(query, (value, key) => {
					...
					result = this.postBuild.call(this.__builderInstance__, result, type, itemType);
