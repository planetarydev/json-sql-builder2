...
function buildRelativePath(sourcePath, destPath) {
	let dirsBack = destPath.split('/').length;
	let relativePath = '';
	...
	if (typeDef.syntax.__registeredHelpers__) {
		...
		_.forEach(typeDef.syntax.__registeredHelpers__, (helper, helperName) => {
			...
			if (!sql._operators[helperName]) {
				...
				// current path = ./sql/operators/select/private/where/where.js
				helperLink = `[${helperName.replace('$', '')}](${buildRelativePath(m.modulePath, sql._operators[helperName].sqlPath)}/)`; //    ../sql/helpers/logical/and/and.js
			}
