...
module.exports = class Syntax {
	...
	__replaceSyntaxWithValues__(syntax, rplv) {
		_.forEach(['key', 'value', 'identifier'], (placeholder) => {
			...
			/*XXX TESET: syntax = this.__replaceRequiredPlaceholder__(syntax, `<${placeholder}-param>`, rplv[placeholder], (value) => {
				if (_.isPlainObject(value)){
					return this.__builderInstance__._queryUnknown({$:value});
				}
				return this.__builderInstance__.addValue.bind(this.__builderInstance__);
			});*/
	...
	__buildUnknown__(query, identifier, preIdentifier) {
		...
		_.forEach(query, (value, key) => {
			...
			if (this.__builderInstance__.isOperator(key)) { //(key.startsWith('$')) {
