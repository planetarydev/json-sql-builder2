...
module.exports = class Syntax {
	...
	__createFunctionCallee__(){
		...
		this.__helperInstance__.callee = (...args) => {
			...
			// we got no helpers
			// but, check the type for an irterateable type
	...
	__replaceSyntaxWithValues__(syntax, rplv) {
		...
		/*if (syntax.indexOf('<key>') > -1) {
			if (_.isArray(rplv.key)){
				syntax = syntax.replace('<key>', this.__buildArrayValue__(rplv.key, function(value){
					return value;
				}));
			} else {
		...
		if (syntax.indexOf('<key-param>') > -1) {
			if (_.isArray(rplv.key)){
				syntax = syntax.replace('<key-param>', this.__buildArrayValue__(rplv.key, this.__builderInstance__.addValue));
			} else {
		...
		if (syntax.indexOf('<key-ident>') > -1) {
			if (_.isArray(rplv.key)){
				syntax = syntax.replace('<key-ident>', this.__buildArrayValue__(rplv.key, this.__builderInstance__.quote));
			} else {
		...
		if (syntax.indexOf('<value>') > -1) {
			if (_.isArray(rplv.value)){
				syntax = syntax.replace('<value>', this.__buildArrayValue__(rplv.value, function(value) {
					return value;
				}));
			} else {
		...
		if (syntax.indexOf('<value-param>') > -1) {
			if (_.isArray(rplv.value)){
				syntax = syntax.replace('<value-param>', this.__buildArrayValue__(rplv.value, this.__builderInstance__.addValue));
			} else {
		...
		if (syntax.indexOf('<value-ident>') > -1){
			if (_.isArray(rplv.value)){
				syntax = syntax.replace('<value-ident>', this.__buildArrayValue__(rplv.value, this.__builderInstance__.quote));
			} else {
		...
		if (syntax.indexOf('<identifier>') > -1){
			if (_.isArray(rplv.identifier)){
				syntax = syntax.replace('<identifier>', this.__buildArrayValue__(rplv.identifier, this.__builderInstance__.quote));
			} else {
