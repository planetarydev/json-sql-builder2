...
class SQLBuilder {
	constructor(languageModule, options) {
		...
		// will be set from the languageModule
	...
	isPrimitive(value) {
		...
		return (value === null || _.isString(value) || _.isNumber(value) || _.isBoolean(value));
	}
	 ...
	 * For this you purpose can use the standard method that will do the job for you.
	 *
	...
	quote(identifier){
		if (!this.isString(identifier)) {
			throw new Error('Using quoted identifiers - arguments provided must always be type of String, but got arg1="' + typeof identifier + '"')
		}
		...
		// maybe the identifier includes the table name or the identifier arg is tebl schema and table
		// like <table>.<column> or <schema>.<table>
		// so we have to replace the "." and quote it correctly before and after the quote
		identifier = identifier.split('.').join(this._quoteCharRight + '.' + this._quoteCharLeft);

		return (identifier === '*' || identifier === 'ALL' ? identifier : this._quoteCharLeft + identifier + this._quoteCharRight);
	}
	...
	addValue(val){
		...
		if (this.isPostgreSQL() && this.isCurrent('$create')) {
	...
	defineKeyword(keyword) {
		...
		if (this._options.attachGlobal === true) {
			global[kw] = this[kw];
		}
	}
	...
	_loadModules(){
		...
		_.forEach(modules, (module) => {
			if (module.endsWith('.js') && module.indexOf('private') == -1) {
	...
	_register(file) {
		...
		if (!sqlModule.definition) {
			let modulePath = file.split('/sql');
			let moduleDisplayPath = (modulePath && modulePath.length == 2 ? '/sql' + modulePath[1] : file);
		...
		let operator = '$' + operatorClass.name;
		...
		// remove the filename
		this.currentModulePath = this.currentModulePath.split('/');
		this.currentModulePath.pop();
		this.currentModulePath = this.currentModulePath.join('/');
		...
		if (_.isFunction(this._operators[operator].callee)) {
			...
			if (this._options.attachGlobal === true) {
				global[operatorClass.name.toLowerCase()] = this[operatorClass.name];
			}
		}
	}
