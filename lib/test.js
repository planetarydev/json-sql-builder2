...
let myQuery = SELECT({
    people_name: $concat($left($i('people.first_name'), 1), '. ', $i('people.last; DROP SCHEMA public CASCADE --Name')),
    $from: 'people',
	$where: {
...
let updatePeople = $update('people', {
	$set: {
		last_name: 'Doe',
		first_name: 'John',
		age: { $add: 1 }
...
class Test {
	...
	run(testForSpecificLang, testForSpecificModule){
		...
		_.forEach(this.modules, (moduleName) => {
			if (testForSpecificModule && testForSpecificModule !== moduleName.split('/sql')[1]) return;
