...
class Test {
	constructor(){
		...
		this.modules = modules.filter(function(module){
			return module.endsWith('.js') && module.indexOf('_private') == -1;
		});
	...
	getModulePath(modulePath){
		let moduleNames = modulePath.split('/sql');
	...
	compareTestResult(depth, path, language, testName, testCase, result, log) {
		...
		if (result.sql !== testCase.expectedResults.sql) {
			...
			log.error(path, language, '  ' + colors.green('  +|' + testCase.expectedResults.sql + '|'));
			success = false;
		}

		// check length of values equals expected
		let expectedValues = testCase.expectedResults.values || {};
	...
	run(){
		console.log('Running tests for each Module located in ./sql\n');
		...
		_.forEach(this.modules, (moduleName) => {
			...
			// test each module for each language
			let supportedLanguages = SQLBuilder.supportedLanguages; // ['PostgreSQL'] //
			_.forEach(supportedLanguages, (language) => {
				...
				try {
					...
					if (instance === undefined){
						...
						// is ../sql/.....
						sql.currentModulePath = path.dirname(path.join('../sql', moduleName.split('/sql')[1]));
